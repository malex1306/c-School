...
class Program
{
    static void Main()
    {
        ...
        Console.WriteLine($"Die Summer dieser Zahlen ist{summe}");

        // Berechnung des Durchschnitts
        Console.WriteLine("Berechnung des Durchschnittswerts:");
        Console.WriteLine($"Durschnittwert = {durchschnitt:F2}");
        ...
        Console.WriteLine($"Gröster Wert = {groessterWert}");

        // Häufigkeitsanalyse
        Console.WriteLine("Häufigkeit der Werte:");
        foreach (var item in haeufigkeit)
        {
            Console.WriteLine($"Zahl {item.Key}: {string.Join(" | ", Enumerable.Repeat(item.Key, item.Value))};");
        }
    }
